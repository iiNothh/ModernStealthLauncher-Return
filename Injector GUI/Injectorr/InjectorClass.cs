...
{
    internal class InjectorClass
    {
        internal static bool succesfuly = false;
        ...
        internal void Main(string[] args)
        {
            ...
            if (!IsElevated)
            {
                Logger.Instance.Warning("Injector Not Running With Administrator Privileges");
            ...
            {
                Logger.Instance.Warning("Antivirus Detected");
            }
            ...
            if (!inject && !eject)
            {
                Logger.Instance.Error("No operation (inject/eject) specified");
                return;
            }
            ...
            {
                Logger.Instance.Error("No process id/name specified");
                return;
            }
        ...
        private static void Inject(Injector injector, CommandLineArguments args)
        {
            ...
            if (args.GetStringArg("-a", out assemblyPath))
            {
                ...
                {
                    Logger.Instance.Error("Could not read the file " + assemblyPath);
                    return;
                }
            }
            else
            {
                Logger.Instance.Error("No assembly specified");
                return;
            }
            ...
            {
                Logger.Instance.Error("No class name specified");
                return;
            }

            if (!args.GetStringArg("-m", out methodName))
            {
                Logger.Instance.Error("No method name specified");
                return;
            }
            ...
            using (injector)
            {
                ...
                try
                {
                    ...
                    Logger.Instance.Success("Injection Succesfuly");
                    succesfuly = true;
                }
                catch (InjectorException ie)
                {
                    Logger.Instance.Error("Failed to inject assembly: " + ie);
                }
                catch (Exception exc)
                {
                    Logger.Instance.Error("Failed to inject assembly (unknown error): " + exc);
                }

                if (remoteAssembly == IntPtr.Zero)
                    return;

                Logger.Instance.Info($"{Path.GetFileName(assemblyPath)}: " + (injector.Is64Bit ? $"0x{remoteAssembly.ToInt64():X16}" : $"0x{remoteAssembly.ToInt32():X8}"));
            }
        }
        ...
        private static void Eject(Injector injector, CommandLineArguments args)
        {
            ...
            {
                Logger.Instance.Error("No assembly pointer specified");
                return;
            }
            ...
            using (injector)
            {
                try
                {
                    ...
                    Logger.Instance.Success("Ejection Succesfuly");
                }
                catch (InjectorException ie)
                {
                    Logger.Instance.Error("Ejection failed: " + ie);
                }
                catch (Exception exc)
                {
                    Logger.Instance.Error("Ejection failed (unknown error): " + exc);
                }
            }
        }
